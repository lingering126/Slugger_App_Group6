...
router.post('/', authMiddleware, async (req, res) => {
  try {
    const {
      ...
      description,
      targetName,
      weeklyLimitPhysical = 100,
      weeklyLimitMental = 100
    } = req.body;
...
router.get('/:teamId', authMiddleware, async (req, res) => {
  try {
    ...
    console.log(`User ${userId} fetching team with ID: ${teamId}`);
    ...
    console.log(`Successfully fetched team ${teamId}`);
    res.status(200).json(team);
  } catch (error) {
...
router.put('/:teamId/targets', authMiddleware, async (req, res) => {
  try {
    const { teamId } = req.params;
    const { targetName, weeklyLimitMental, weeklyLimitPhysical } = req.body;
    const userId = req.user.userId;
    
    console.log(`User ${userId} attempting to update targets for team ${teamId}`); 
    console.log('Update data:', { targetName, weeklyLimitMental, weeklyLimitPhysical });
    ...
    // Update weekly limits if provided
    if (weeklyLimitMental !== undefined) {
      team.weeklyLimitMental = weeklyLimitMental;
      console.log(`Updated mental weekly limit to ${weeklyLimitMental}`);
    }
    
    if (weeklyLimitPhysical !== undefined) {
      team.weeklyLimitPhysical = weeklyLimitPhysical;
      console.log(`Updated physical weekly limit to ${weeklyLimitPhysical}`);
    }
